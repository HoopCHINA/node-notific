...
NotificAgent.prototype.notific = function (appid, tokens, payload, expiry) {
  ...
  if (!channel) {
    ...
    channel.on('invtoken', function (invtoken) {
      self.emit('invtoken', appid, invtoken);
    });
  }
...
function PushChannel(agent, appid, opts) {
  ...
  this.maxcache = popts.maxcache || 100;
...
PushChannel.prototype._flush = function () {
  ...
  } else if (this.state == STATE_LOADED || this.state == STATE_CLOSED) {
    this._connect();
