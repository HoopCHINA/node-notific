...
NotificServer.prototype._handler = function (socket) {
  ...
  parser.on('connect', function (packet) {
    ...
    // Check client
    if (!_validateClient(packet.client)) return _endWithAck(2);
...
// `clients` device ids
// `expiry` per minute
NotificServer.prototype.notific = function (app, clients, payload, expiry) {
  // Check expiry
  ...
  clients.forEach(function (client) {
    self._notific(app, client, mid, payload, expiry);
  });
};

NotificServer.prototype._notific = function (app, client, mid, payload, expiry) {
